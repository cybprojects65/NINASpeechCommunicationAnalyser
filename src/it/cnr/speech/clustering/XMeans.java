...
public class XMeans 
   ...
   */
  public XMeans(File workingFolder,  HashMap<String,String> labels) {
    super();
  ...
  public void compute(int minpoints, int minClusters, int maxClusters, int maxIterations, double[][]data) throws Exception{
	  
	  
		System.out.println("XMeans: Setting up the cluster");
		
		StringBuffer sb = new StringBuffer();
		int nrowsd = data.length;
		int ncolsd = data[0].length;
		for (int i = -1; i < nrowsd; i++) {
			for (int j = 0; j < ncolsd; j++) {
				if (i==-1)
					sb.append("F"+j);
				else	
					sb.append(data[i][j]);
		...
		setMinNumClusters(minClusters);
		setMaxNumClusters(maxClusters);
		buildClusterer(id);
		
		// do clustering
		System.out.println("XMeans: Clustering ...");
		Instances is = getClusterCenters();
		int nClusters = is.numInstances();
		// take results
		System.out.println("XMeans: Found "+nClusters+" Centroids");
		/*
		for (Instance i : is) {
			DenseInstance di = (DenseInstance) i;
			int nCluster = di.numAttributes();
			for (int k = 0; k < nCluster; k++) {
					System.out.println("K:"+k+"->"+di.toString(k));
			}
			System.out.println("-------------------------------");
		}
		*/
		int[] clusteringAssignments = m_ClusterAssignments;
		...
		String columnsNames = "audio_id,audio_file,speaker_id,is_an_outlier\n";
		...
		System.out.println("Analyzing Cluster ->" + "Building Rows to Insert");
		...
		for (int k = 0; k < nrows; k++) {
			...
			bufferRows.append(k+","+labels.get(""+(k+1))+","+cindex + "," + isoutlier+"\n");
		}
		
		System.out.println("Analyzing Cluster ->" + "Inserting rows");
		
		if (bufferRows.length() > 0) {
			System.out.println("XMeans: Writing into DB");
			System.out.println("Writing into DB");
			File output = new File(workingFolder,EnergyPitchClusterer.CLUSTERINGFILE);
			FileWriter fw = new FileWriter(output);
			fw.write(bufferRows.toString());
			fw.close();
			System.out.println("XMeans: Finished with writing into DB");
		} else
			System.out.println("XMeans: Nothing to write in the buffer");
