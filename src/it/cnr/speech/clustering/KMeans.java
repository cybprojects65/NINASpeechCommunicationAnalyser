...
public class KMeans {
	...
	public KMeans(File workingFolder, HashMap<String,String> labels) {
		this.workingFolder = workingFolder;
		this.labels = labels;
		initRapidMiner();
	}	
		
	public File compute(int kk, int maxRuns, int maxOptimizations, int minPoints, double[][] features) throws Exception {
		...
		File outputTable = buildClusterTable(outputvector);
		return outputTable;
	}
	
	
	protected File buildClusterTable(IOObject[] outputvector) throws Exception{
		...
		System.out.println("Analyzing Cluster -> found "+ nClusters+" clusters");
		System.out.println("Start Write On DB");
		StringBuffer bufferRows = new StringBuffer();
		int allCounter = 0;
		bufferRows.append("audio_id,audio_file,speaker_id,is_an_outlier\n");
